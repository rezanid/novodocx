...
namespace Novo.Docx.Cli.Utils
{
    ...
    {
        private static readonly char[] s_pathSeparator = new char[] { Path.PathSeparator };
        private static readonly char[] s_quote = new char[] { '"' };
        ...
        private string ExpandTildeSlash(string path)
        {
            ...
            {
                return Path.Combine(_userHomeDirectory.Value, path[tildeSlash.Length..]);
            }
            else
            {
                return path;
